import './AssignmentActivityItem.scss';
...
const hasName = type =>
	(t(type, { name: type, title: '' }) || '').indexOf(type) >= 0;
...
export default class ActivityItem extends React.Component {
	...
	getLabelWithUser = data =>
		t(getType(this.props.event), { ...data, title: this.getTitleMarkup() });
		...
		return (
			<div className={cx('item', { unread })}>
				<a href={href}>
					...
					{hasName(type) ? (
						<DisplayName
							entity={feedbackAuthor || user}
							usePronoun
							localeKey={this.getLabelWithUser}
						/>
					) : (
						<span
							{...rawContent(
								t(type, { title: this.getTitleMarkup() })
							)}
						/>
					)}
				</a>
