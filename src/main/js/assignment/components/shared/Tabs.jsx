...
class Tabs extends React.Component {
	...
	render() {
		...
		const disableActivityView =
			isAdministrative && !course.hasLink('CourseActivity');

		const PerformanceLink = disablePerfView ? 'a' : ActiveState;
		const PerformanceLinkProps = disablePerfView
			? { className: 'disabled' }
			: { tag: 'a', href: '/performance/', hasChildren: true };
		const ActivityLink = disableActivityView ? 'a' : ActiveState;
		const ActivityLinkProps = disableActivityView
			? { className: 'disabled' }
			: { tag: 'a', href: '/activity/' };
			...
			<div className="assignments-nav">
				...
				<ActiveStateContainer>
					<ul className="filters">
						<li>
							<ActiveState tag="a" href="/" hasChildren>
								{t('assignments')}
							</ActiveState>
						</li>
						<li>
							<PerformanceLink {...PerformanceLinkProps}>
								{t('gradesAndPerformance')}
							</PerformanceLink>
						</li>
						<li>
							<ActivityLink {...ActivityLinkProps}>
								{t('activity')}
							</ActivityLink>
						</li>
					</ul>
...
export default decorate(Tabs, [Assignments.connect]);
