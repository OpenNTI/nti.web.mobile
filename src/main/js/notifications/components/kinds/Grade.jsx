...
export default createReactClass({
	...
	statics: {
		noteableType: ['gradebook.grade', 'grade'],
	},
	...
	async componentDidMount() {
		const {
			item: {
				Item: { creator, CatalogEntryNTIID },
			},
		} = this.props;
	...
	render() {
		const {
			state: { url, catalogEntry, creator },
			props: {
				item: {
					Item: { CourseName, AssignmentName = 'an assignment' },
				},
			},
		} = this;
		...
		return (
			<li className="notification-item">
				<a href={url}>
					...
					<div className="wrap">
						{creator && (
							<DisplayName entity={creator} suppressProfileLink />
						)}
						{catalogEntry && <span>{catalogEntry.Title}</span>}
						<span>
							{' '}
							{t('grade')} {AssignmentName}
