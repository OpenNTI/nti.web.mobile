...
export default class CourseLessonOverview extends React.Component {
	...
	getRouteFor = (obj, context) => {
		...
		if (/video/i.test(type)) {
			...
			route = path.join(
				'..',
				'discussions',
				encodeForURI(obj.ContainerId),
				encodeForURI(obj.NTIID)
			);
			...
			route =
				context === 'discussions'
					...
					? {
							href: obj.href,
							target: obj.isExternal ? '_blank' : void 0,
					  }
					: path.join(
							env.getPath(),
							'content',
							encodeForURI(obj.href)
					  ) + '/';
			...
			return {
				href: `${obj.getLink('Launch')}?target=window`,
				target: '_blank',
			};
		} else {
	...
	render() {
		...
		const render = () => {
			...
			return <EmptyList type="lesson-overview" />;
		};

		return (
			<ContextSender
				ref={this.attachContextProviderRef}
