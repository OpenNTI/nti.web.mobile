...
registerGenerator(
	...
	async (service, context, object) => {
		const survey =
			object instanceof SurveyRefModel
				? context.getInquiry
					? await context.getInquiry(object.target)
		...
		const {
			content,
			AssessmentItems,
		} = await Survey.Viewer.Utils.createPageInfo(survey, context);
...
export function loadPage(ntiid, context, extra) {
	return load(ntiid, context, extra)
		.then(packet => (dispatch(PAGE_LOADED, packet), packet))
		.catch(
			error => (
				dispatch(PAGE_FAILED, { error, ntiid }), Promise.reject(error)
			)
		);
}
	...
	return getService()
		...
		.catch(x => {
			...
			if (code === 501 || code === 422) {
				logger.log(
					'Ignored condition. Either the link did not exist, or has not been adapted to be generic.'
				);
...
export { getPageContent, getPackage };
