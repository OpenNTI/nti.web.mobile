...
function isWidget(node) {
	return (
		(node.hasAttribute && node.hasAttribute('data-reactid')) ||
...
function validToWrapEntireNode(node) {
	...
	if (DOM.isElement(node)) {
		...
		if (
			...
			((DOM.hasClass(node, 'mathjax') ||
				DOM.hasClass(node, 'mathquill')) &&
...
export default {
	...
	wrapRange(node, range) {
		...
		let startToStart = nodeRange.compareBoundaryPoints(
			Range.START_TO_START,
			range
		);
		let startToEnd = nodeRange.compareBoundaryPoints(
			Range.START_TO_END,
			range
		);

		let endToStart = nodeRange.compareBoundaryPoints(
			Range.END_TO_START,
			range
		);
