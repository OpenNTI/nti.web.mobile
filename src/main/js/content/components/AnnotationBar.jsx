...
export default class AnnotationBar extends React.Component {
	...
	render() {
		...
		const hightlighters =
			onSetHighlight &&
			['Yellow', 'Green', 'Blue', 'Blackout'].map(x => (
				<Button
					key={x}
					data-color={x}
					className={cx('ugd highlight', x.toLowerCase(), {
						selected:
							(item === x.toLowerCase() && !busy) ||
							busy === x.toLowerCase(),
		...
		return (
			<div className="add annotation toolbar" ref={this.ref}>
				...
				{!!onNewDiscussion && (
					<Button className="ugd note" onClick={this.onNote}>
				...
				{!!discussionLink && (
					<Link className="ugd note" href={discussionLink}>
						<i className="icon-discuss small" />
						View Discussion
					</Link>
				)}
