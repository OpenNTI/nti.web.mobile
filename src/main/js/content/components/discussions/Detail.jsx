...
export default createReactClass({
	...
	render() {
		const {
			state: { replying },
			props: { contentPackage, item, lite },
		} = this;
		const { body } = item;
		...
		return (
			<div className={`discussion-${isReply ? 'reply' : 'detail'}`}>
				<div className="root">
					{!item.placeholder && (
						<AuthorInfo item={item} lite={!isReply && lite} />
					)}

					{!lite && (
						<Context item={item} contentPackage={contentPackage} />
					)}
	...
	renderReply(item) {
		return <Reply item={item} key={item.getID()} />;
	},
});
