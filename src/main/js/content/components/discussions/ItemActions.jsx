...
const CanShare = (caps, item) =>
	CanEdit(caps, item) && item.isTopLevel && caps && caps.canShare && false; //disable for now
...
export default createReactClass({
	...
	render() {
		...
		const showMenu = [CanReply, CanShare, CanEdit, CanDelete].some(x =>
			x(capabilities, item)
		);
		...
		return showMenu ? (
			<div className="discussion-item-actions">
				<Action
					name="reply"
					criteria={CanReply(capabilities, item)}
					onClick={this.onReply}
				/>
				<Action
					name="share"
					criteria={CanShare(capabilities, item)}
				...
				>
					<ul className="discussion-item-actions-options">
						<Action
							name="edit"
							criteria={CanEdit(capabilities, item)}
							inList
							onClick={this.onEdit}
						/>
						<Action
							name={flag}
							criteria={CanFlag(capabilities, item)}
							inList
							onClick={this.onFlag}
						/>
						<Action
							name="delete"
							criteria={CanDelete(capabilities, item)}
							inList
