...
import { View as Library } from '@nti/web-library';
	...
	return (
		<NavigationBar {...props} theme={theme}>
			<div className="library-branding-container" position="left">
				<Theme.Asset
					className="library-branding-asset"
...
class Home extends React.Component {
	...
	getRouteFor = (obj, context) => {
		...
		const getEncodedIDCourse = o =>
			encodeForURI(o.getLinkProperty('CourseInstance', 'ntiid'));
		const { MimeType: type } = obj || {};

		let route = '';

		if (
			/course(.*)enrollment/i.test(type) ||
			/courseinstanceadministrativerole/i.test(type)
		) {
			route = path.join(basePath, 'course', getEncodedIDCourse(obj));
		} else if (/contentpackagebundle/i.test(type)) {
			route =
				getRememberedRoute(obj) ||
				path.join(basePath, 'content', encodeForURI(obj.getID()));
...
export default decorate(Home, [searchable(), contextual('Home')]);
