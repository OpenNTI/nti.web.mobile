...
export function updatePartsWithAssessedParts(part, assessed) {
	...
	let questions = assessed.getQuestions
		? assessed.getQuestions()
		: [assessed];
	...
	questions.forEach(q => {
		...
		if (!question || q.parts.length !== question.parts.length) {
			logger.error(
				'We have an assessed value, but no where to put it. (Question is missing or has the wrong number of parts)'
			);
			return;
		}
 ...
 *
 * @returns {boolean} true if it doesn't match
 */
export function areAssessmentsSupported() {
	let nua = navigator.userAgent;
	let isAndroidNative =
		/Mozilla\/5\.0/.test(nua) &&
		/Android /.test(nua) &&
		/AppleWebKit/.test(nua) &&
		!/Chrome/.test(nua);

	let isAndroidFireFox =
		/Mozilla\/5\.0/.test(nua) &&
		/Android/.test(nua) &&
		/Firefox/i.test(nua);
