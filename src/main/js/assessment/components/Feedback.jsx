...
const t = scoped('assessment.assignment.feedback', {
	title: 'Feedback',
	description:
		'The comments below will only be visible to you and your instructor.',
});
...
export default createReactClass({
	...
	onSubmit(feedbackBody) {
		return submitFeedback(this.props.assessment, feedbackBody).then(
			() => this.forceUpdate(),
			logError
		);
	},
	...
	onEditItem(item, newValue) {
		return updateFeedbackItem(this.props.assessment, item, newValue).then(
			() => this.forceUpdate(),
			logError
		);
	},
	...
	onDeleteItem(item) {
		return deleteFeedbackItem(this.props.assessment, item).then(
			() => this.forceUpdate(),
			logError
		);
	},
