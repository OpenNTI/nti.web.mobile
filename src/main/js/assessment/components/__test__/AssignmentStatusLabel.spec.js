...
describe('AssignmentStatusLabel', () => {
	...
	describe('Unpublished States', () => {
		...
		test('Unpublished', () => {
			...
			const value = 'Draft';
			const overtime = Object.assign(Object.create(assignment), {
				isOverTime: () => true,
			});
	...
	describe('Unsubmitted States', () => {
		...
		test('Base Cases: overdue / overtime should not change until submitted.', () => {
			...
			const historyItem = MockAssignmentHistory(
				new Date('2015-08-29T05:00:00Z')
			);

			const due = DateTime.format(
				assignment.getDueDate(),
				EXPECTED_DAY_FORMAT
			);
			const completed = DateTime.format(
			...
			testRender({ assignment, historyItem })
				.map(getText)
				.forEach(text =>
					expect(text).toEqual(`Completed(Overdue)${completed}`)
				);
			...
			testRender({ assignment: overtime, historyItem })
				...
				.forEach(text =>
					expect(text).toEqual(
						`Completed(OvertimeOverdue)${completed}`
					)
				);
		});
		...
		test('Base Cases: Available verbage', () => {
			...
			testRender({ assignment })
				.map(getText)
				.forEach(text =>
					expect(text).toEqual(`Available()Today at ${hours}`)
				);
			...
			//no submit
			assignment = MockNoSubmitAssignment({
				[START]: tomorrow,
				[END]: void 0,
			});
			time = DateTime.format(
	...
	describe('Overdue', () => {
		...
		test('Base Cases: should show time with date', () => {
			...
			const completed = DateTime.format(
				historyItem.getCreatedTime(),
				EXPECTED_DAYTIME_FORMAT
			);
			const value = `Completed(Overdue)${completed}`;

			const [A, B] = testRender({
				assignment,
				historyItem,
