...
export default class SwipeToRevealOptions extends React.Component {
	...
	static defaultProps = {
		...
		parentWidth:
			...
			(typeof global.screen !== 'undefined' && global.screen.width) ||
			320,
	};
			...
			<div className={classes} style={this.getContainerStyle()}>
				<div className="stro-left">
					{leftOptions.map((option, index) => {
						const propsLabel = {
							style: this.getSpanStyle('left', index),
						};
				...
				<div className="stro-right">
					{rightOptions.map((option, index) => {
						const propsLabel = {
							style: this.getSpanStyle('right', index),
						};
						if (typeof option.label === 'string') {
							propsLabel.dangerouslySetInnerHTML = {
								__html: option.label,
						...
						return (
							<div
								...
								onClick={() => this.rightClick(option)}
	...
	shouldTransitionBack(direction) {
		if (
			(direction === 'right' && this.state.showRightButtons) ||
			this.state.showLeftButtons
		) {
			this.transitionBack();
		}
	...
	shouldCloseOthers(direction) {
		if (this.props.closeOthers) {
			if (
				(direction === 'right' && !this.state.swipingRight) ||
	...
	swiped = () => {
		switch (this.state.action) {
			...
			case 'rightAction':
				this.rightClick(
					this.props.rightOptions[this.props.rightOptions.length - 1]
				);
				break;
		}
	...
	rightClick = option => {
		...
		if (this.props.transitionBackOnRightClick) {
			this.transitionBack();
		}
	};
	...
	leftClick = option => {
		...
		if (this.props.transitionBackOnLeftClick) {
			this.transitionBack();
		}
	};
	...
	getItemWidth(side) {
		let nbOptions =
			side === 'left'
				...
				: this.props.rightOptions.length;
		return Math.min(
			this.props.parentWidth / (nbOptions + 1),
	...
	getStyle(side, index) {
		...
		if (
			...
			offset +=
				factor *
				(Math.abs(this.state.delta) - nbOptions * width) *
				0.85;
		}
