...
import { LINK_ACCOUNT_CREATE, MESSAGE_SIGNUP_CONFIRMATION } from '../Constants';
...
function getWrapper(node) {
	...
	while (current) {
		if (current.matches('.login-wrapper')) {
			return current;
		}
...
const signupLink = () =>
	t(MESSAGE_SIGNUP_CONFIRMATION, { fallback: 'missing' }) === 'missing'
		? '/signup/'
		: '/signup/confirm';
...
export default createReactClass({
	...
	attachUsernameInputRef(el) {
		this.username = el;
	},
	attachPasswordInputRef(el) {
		this.password = el;
	},
	...
	formatError(error) {
		let message =
			typeof error === 'string'
				? error
				: t(`LOGIN_ERROR.${(error || {}).statusCode}`, {
						fallback: 'missing',
				  });
	...
	onClick(e) {
		...
		if (
			Math.abs(rect.right - clientX) <= 40 &&
			Math.abs(rect.bottom - clientY) <= 40
		) {
			this.setState({
	...
	render() {
		...
		const hasNextThoughtLogin =
			forceNextThoughtLogin || !Store.hasOAuthLinks || hasAccountCreation;

		return (
			<div
				className={cx('login-wrapper', {
					'nextthought-login': hasNextThoughtLogin,
				})}
						...
						<div>
							...
							<fieldset>
								{hasNextThoughtLogin && (
									...
									>
										<input
											ref={this.attachUsernameInputRef}
											name="username"
											type="text"
								...
								{hasNextThoughtLogin && (
									...
									>
										<input
											ref={this.attachPasswordInputRef}
											name="password"
											type="password"
											autoComplete="off"
	...
	updateUsername(e) {
		...
		if (username) {
			//normal case, we have an element.
			username = username.value.trim(); // flatten down to a string.
		} else if (!e) {
			// submit case, no element, only state:
		...
		if (e) {
			// from an dom event and its not expecting a return value, we need to return nothing.
