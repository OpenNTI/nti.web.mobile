...
export default class PasswordResetForm extends React.Component {
	...
	onInput = () => {
		...
		let submitEnabled = Object.values(fields).every(
			x => x && x.trim().length > 0
		);
	...
	handleSubmit = e => {
		...
		this.setState({ error: null, busy: true }, () => {
			...
			getServer()
				.resetPassword(decodeURIComponent(username), password, token)
				...
				>
					...
					{!busy && !success && (
						<div>
							...
							{!error && (
								<div className="message recover green">
					...
					{!!success && !busy && (
						<fieldset className="success">
							...
							<Link
								id="login:return"
								href="/"
								className="fi-arrow-left return-link"
							>
								{' '}
								Return to Login
							</Link>
