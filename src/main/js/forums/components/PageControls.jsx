	...
	pageSelector(paging) {
		...
		return (
			<div className="page-selector-wrapper">
				...
				>
					{Array.from({ length: paging.numPages }).map(
						(_, i) => (
							i++,
							(
	...
	render() {
		...
		return (
			<ul className="page-controls">
				<li className="item previous">
					{paging.hasPrevious && (
						<Link className="link" href={'/?page=' + prev}>
							Previous
						</Link>
					)}
				</li>
				<li className="item current">
					{paging.numPages > 1
						? this.pageSelector(paging)
						: 'Page 1 of 1'}
				</li>
				<li className="item next">
					{paging.hasNext && (
						<Link className="link" href={'/?page=' + next}>
							Next
						</Link>
					)}
				</li>
