...
import { GOT_COMMENT_REPLIES, COMMENT_ADDED, ITEM_DELETED } from '../Constants';
...
export default createReactClass({
	...
	onReceivedCommentReplies(event) {
		...
		if (event.comment === item) {
			...
			this.setState({
				replies: (event.replies || []).filter(
					x => x.inReplyTo === itemId
				),
			});
		}
	},
	...
	renderReplies() {
		const {
			props: { display, listComponent: List, ...props },
			state: { replies },
		} = this;
		...
		return items.length > 0 ? (
			<List container={{ Items: items }} {...props} />
		) : null;
	},
	...
	render() {
		...
		return (
			<div className={className}>
				<div className="replies">{this.renderReplies()}</div>
			</div>
		);
	},
});
