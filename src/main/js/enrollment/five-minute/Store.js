...
function preflightAndSubmit(action) {
	...
	preflight(input)
		.then(
			...
			reason => {
				Store.emitError({
					type: Constants.PREFLIGHT_ERROR,
					action,
					reason,
				});
		...
		.then(
			response =>
				Store.emitChange({
					type: Constants.ADMISSION_SUCCESS,
					action,
					response,
				}),
			reason => {
				...
				Store.emitError({
					type: Constants.REQUEST_ADMISSION_ERROR,
					action,
					reason,
				});
				return Promise.reject(reason);
...
function requestAdmission(data) {
	return getFiveMinuteService().then(service =>
		service.requestAdmission(data)
	);
}

function requestConcurrentEnrollment(action) {
	let requestEnrollment = getFiveMinuteService().then(service =>
		service.requestConcurrentEnrollment(action)
	);

	requestEnrollment.then(
		response =>
			Store.emitChange({
				type: Constants.CONCURRENT_ENROLLMENT_SUCCESS,
				action,
				response,
			}),
		reason =>
			Store.emitError({
				type: Constants.CONCURRENT_ENROLLMENT_ERROR,
				action,
				reason,
			})
	);
...
function externalPayment(action) {
	...
	if (!data.ntiCrn || !data.ntiTerm || !data.returnUrl) {
		throw new Error(
			'action payload requires ntiCrn and ntiTerm parameters. Received %O',
			data
		);
	}

	let getUrl = getExternalPaymentUrl(
		data.link,
		data.ntiCrn,
		data.ntiTerm,
		data.returnUrl
	...
	getUrl.then(
		response => {
			if (response.rel === 'redirect') {
				Store.emitChange({
					type: Constants.RECEIVED_PAY_AND_ENROLL_LINK,
					action,
					response,
				});
			}
		...
		(reason = {}) => {
			Store.emitError({
				type: Constants.PAY_AND_ENROLL_ERROR,
				action,
				reason,
			});
			return Promise.reject(reason);
