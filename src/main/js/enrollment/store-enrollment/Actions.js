...
export function verifyBillingInfo (stripePublicKey, formData, createToken) {
	...
	function pullData (data) {
		...
		const {coupon, expected_price} = data; // eslint-disable-line camelcase
		const couponInfo = { coupon, expected_price }; // eslint-disable-line camelcase
		...
		const giftInfo = data.from && Object.keys(result).length ? result : null;
	...
	createToken(formData)
		...
		.catch(result => dispatch(Constants.BILLING_INFO_REJECTED, {formData, result}));
}
...
export function submitPayment (formData) {
	return Api.submitPayment(formData)
		.then(result => {
			const eventType = (result || {}).state === 'Success' ?
...
export function updateCoupon (purchasable, coupon) {
	...
	updateCoupon.timeout = setTimeout(() => {
		return Api.getCouponPricing(purchasable, coupon)
			.then(
